...
class TypeAdmin extends AdminController
{
    ...
    {
        if (IS_POST) {
            $_POST['list_field']   = isset($_POST['list_field']) ? implode(',', $_POST['list_field']) : '';
            $_POST['filter_field'] = isset($_POST['filter_field']) ? implode(',', $_POST['filter_field']) : '';
            $type_object           = D('Type');
            $data                  = $type_object->create();
            if ($data) {
            ...
            foreach ($document_type_field_sort as $key => $val) {
                ...
                $temp                 = array();
                foreach ($val as $val2) {
                    if (isset($new_attribute_list[$val2])) {
