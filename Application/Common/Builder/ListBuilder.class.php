    ...
    public function addRightButton($type, $attribute = null)
    {
        switch ($type) {
            ...
            case 'forbid': // 改变记录状态按钮，会更具数据当前的状态自动选择应该显示启用/禁用
                ...
                $this->_right_button_list[] = $my_attribute;
                break;
            case 'check': // 改变记录状态按钮，会更具数据当前的状态自动选择应该显示启用/禁用
                //预定义按钮属
                $my_attribute['type']             = 'forbid';
                $my_attribute['model']            = $attribute['model'] ?: CONTROLLER_NAME;
                $my_attribute['forbid0']['name']  = 'forbid';
                $my_attribute['forbid0']['title'] = '通过';
                $my_attribute['forbid0']['class'] = 'label label-success-outline label-pill ajax-get confirm';
                $my_attribute['forbid0']['href']  = U(
                    MODULE_NAME . '/' . CONTROLLER_NAME . '/setStatus',
                    array(
                ...
                $this->_right_button_list[] = $my_attribute;
                break;
            case 'top': // 改变记录状态按钮，会更具数据当前的状态自动选择应该显示启用/禁用
                //预定义按钮属
                $my_attribute['type']             = 'top';
                $my_attribute['model']            = $attribute['model'] ?: CONTROLLER_NAME;
                $my_attribute['forbid0']['name']  = 'forbid';
                $my_attribute['forbid0']['title'] = '置顶';
                $my_attribute['forbid0']['class'] = 'label label-success-outline label-pill ajax-get confirm';
                $my_attribute['forbid0']['href']  = U(
                    MODULE_NAME . '/' . CONTROLLER_NAME . '/setStatus',
                    array(
                ...
                $my_attribute['forbid1']['name']  = 'top';
                $my_attribute['forbid1']['title'] = '取消置顶';
                $my_attribute['forbid1']['class'] = 'label label-warning-outline label-pill ajax-get confirm';
                $my_attribute['forbid1']['href']  = U(
                    MODULE_NAME . '/' . CONTROLLER_NAME . '/setStatus',
                    array(
                ...
                $this->_right_button_list[] = $my_attribute;
                break;
            case 'refuse': // 改变记录状态按钮，会更具数据当前的状态自动选择应该显示启用/禁用
                //预定义按钮属
                $my_attribute['type']             = 'forbid';
                $my_attribute['model']            = $attribute['model'] ?: CONTROLLER_NAME;
                $my_attribute['forbid0']['name']  = 'forbid';
                $my_attribute['forbid0']['title'] = '拒绝';
                $my_attribute['forbid0']['class'] = 'label label-danger-outline label-pill ajax-get confirm';
                $my_attribute['forbid0']['href']  = U(
                    MODULE_NAME . '/' . CONTROLLER_NAME . '/setStatus',
                    array(
                        'status' => 'refuse',
                ...
                $my_attribute['forbid1']['name']  = 'forbid';
                $my_attribute['forbid1']['title'] = '恢复';
                $my_attribute['forbid1']['class'] = 'label label-danger-outline label-pill ajax-get confirm';
                $my_attribute['forbid1']['href']  = U(
                    MODULE_NAME . '/' . CONTROLLER_NAME . '/setStatus',
                    array(
                        'status' => 'unrefuse',
                ...
                $this->_right_button_list[] = $my_attribute;
                break;
            case 'classic': // 改变记录状态按钮，会更具数据当前的状态自动选择应该显示启用/禁用
                //预定义按钮属
                $my_attribute['type']             = 'classic';
                $my_attribute['model']            = $attribute['model'] ?: CONTROLLER_NAME;
                $my_attribute['forbid0']['name']  = 'forbid';
                $my_attribute['forbid0']['title'] = '精华';
                $my_attribute['forbid0']['class'] = 'label label-success-outline label-pill ajax-get confirm';
                $my_attribute['forbid0']['href']  = U(
                    MODULE_NAME . '/' . CONTROLLER_NAME . '/setStatus',
                    array(
                ...
                $my_attribute['forbid1']['name']  = 'top';
                $my_attribute['forbid1']['title'] = '取消精华';
                $my_attribute['forbid1']['class'] = 'label label-warning-outline label-pill ajax-get confirm';
                $my_attribute['forbid1']['href']  = U(
                    MODULE_NAME . '/' . CONTROLLER_NAME . '/setStatus',
                    array(
        ...
        foreach ($this->_table_data_list as &$data) {
            ...
            if ($this->_right_button_list) {
                foreach ($this->_right_button_list as $right_button) {
                    ...
                    if (isset($right_button['type']) && $right_button['type'] === 'top') {
                        $right_button = $right_button['forbid' . $data['top']];
                    }
                    if (isset($right_button['type']) && $right_button['type'] === 'top') {
                        if ($data['top'] === '0' || $data['top'] === '1') {
                            $right_button = $right_button['recycle1'];
                        } else {
                            $right_button = $right_button['recycle' . $data['top']];
                        }
                    }
                    if (isset($right_button['type']) && $right_button['type'] === 'classic') {
                        $right_button = $right_button['forbid' . $data['classic']];
                    }
                    if (isset($right_button['type']) && $right_button['type'] === 'classic') {
                        if ($data['top'] === '0' || $data['top'] === '1') {
                            $right_button = $right_button['recycle1'];
                        } else {
                            $right_button = $right_button['recycle' . $data['classic']];
                        }
                    }
            ...
            foreach ($this->_table_column_list as &$column) {
                switch ($column['type']) {
                    ...
                    case 'pictures':
                        ...
                        $pics = '';
                        foreach($temp as $v){
                            $pics .= '<img class="picture" src="' . get_cover($v) . '" style="margin-right:2px;">';
                        }
                        $data[$column['name']] = $pics;
                        break;
                    case 'type':
