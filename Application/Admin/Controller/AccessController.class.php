    ...
    public function edit($id)
    {
        if (IS_POST) {
            ...
            if ($_POST['password'] == '') {
                ...
                $post['password'] =user_md5($post['password']);
            }
            // 提交数据
            $user_object = D('User');
            $data        =$post;
            ...
            $uid =M('admin_access')->where(['id'=>$id])->getField('uid');
            // 获取账号信息
            ...
            ->addFormItem('id', 'hidden', 'ID', 'ID')
                ...
                ->addFormItem('username', 'static', '用户名', '用户名')
                ->addFormItem('password', 'password', '密码', '密码,不填表示不修改')
