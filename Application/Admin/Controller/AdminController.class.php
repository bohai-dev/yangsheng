...
class AdminController extends ControllerController
{
        ...
        switch ($status) {
            ...
            case 'unchecked': //未审条目
                ...
                $this->editRow(
                    ...
                    $data,
                    $map,
                    array('success' => '状态修改成功', 'error' => '状态修改失败')
                );
                break;
            case 'checked': //已审条目
                // $this->checked_sendcustom($ids,$model);
                $data = array('status' => 1);
                $this->editRow(
                    $model,
                    $data,
            ...
            case 'untop':
                ...
                $this->editRow(
                    ...
                    $data,
                    $map,
                    array('success' => '取消置顶成功', 'error' => '取消置顶失败')
                );
                break;
            case 'classic':
                $data = array('classic' => 1);
                $this->editRow(
                    $model,
                    $data,
                    $map,
                    array('success' => '成功', 'error' => '失败')
                );
                break;
            case 'unclassic':
                $data = array('classic' => 0);
                $this->editRow(
                    $model,
                    $data,
                    $map,
                    array('success' => '取消精华成功', 'error' => '取消精华失败')
                );
                break;
            case 'resume': // 启用条目
                $data = array('status' => 1);
    ...
    public function module_config()
    {
        if (IS_POST) {
            ...
            $module_info = D('Admin/Module')->where(array('name' => $name))->find();
